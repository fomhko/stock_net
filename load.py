...
def load_cla_data(data_path, tra_date, val_date, tes_date, seq=5,
    ...
    tra_gt = np.zeros([tra_num, seq,2], dtype=float)
    ins_ind = 0
    for date_ind in range(tra_ind, val_ind):
        # filter out instances without length enough history
        if date_ind < seq:
        ...
        for tic_ind in range(len(fnames)):
            if abs(data_EOD[tic_ind][date_ind][-2]) > 1e-8 and \
                ...
                dir = data_EOD[tic_ind][date_ind + 1 - seq:date_ind + 1, -1] -\
                      data_EOD[tic_ind][date_ind - seq:date_ind,-1]>0
                tra_gt[ins_ind] = np.array([np.array([1, 0]) if i else np.array([0, 1]) for i in dir])
                ins_ind += 1
    ...
    val_gt = np.zeros([val_num, seq,2], dtype=float)
    ...
    for date_ind in range(val_ind, tes_ind):
        ...
        for tic_ind in range(len(fnames)):
            if abs(data_EOD[tic_ind][date_ind][-2]) > 1e-8 and \
                ...
                dir = data_EOD[tic_ind][date_ind + 1 - seq:date_ind + 1, -1] - \
                      data_EOD[tic_ind][date_ind - seq:date_ind, -1] > 0
                val_gt[ins_ind] = np.array([np.array([1, 0]) if i else np.array([0, 1]) for i in dir])
                ins_ind += 1

    # testing
    tes_pv = np.zeros([tes_num, seq, fea_dim], dtype=float)
    tes_wd = np.zeros([tes_num, seq, 5], dtype=float)
    tes_gt = np.zeros([tes_num, seq,2], dtype=float)
    ins_ind = 0
    for date_ind in range(tes_ind, len(trading_dates)):
        # filter out instances without length enough history
        if date_ind < seq:
        ...
        for tic_ind in range(len(fnames)):
            if abs(data_EOD[tic_ind][date_ind][-2]) > 1e-8 and \
                ...
                # tes_gt[ins_ind] = (data_EOD[tic_ind][date_ind - seq:date_ind,-2] + 1) / 2
                dir = data_EOD[tic_ind][date_ind+1 - seq:date_ind+1,-1] - data_EOD[tic_ind][date_ind - seq:date_ind,-1]>0
                tes_gt[ins_ind] = np.array([np.array([1,0]) if i else np.array([0,1]) for i in dir])
                ins_ind += 1
...
if __name__ == '__main__':
    ...
    # if shuffle:
    ind = np.arange(tra_pv.shape[0])
    np.random.shuffle(ind)
    dataset = tra_pv[ind]
    labelset = tra_gt[ind]
    f = open("dataset_tra", 'wb')
    pickle.dump(tra_pv, f)
    f.close()
    f = open("labelset_tra", 'wb')
    pickle.dump(tra_gt, f)
    f.close()
    f = open("dataset_dev", 'wb')
    pickle.dump(val_pv, f)
    f.close()
    f = open("labelset_dev", 'wb')
    pickle.dump(val_gt, f)
    f.close()
    f = open("dataset_test", 'wb')
    pickle.dump(tes_pv, f)
    f.close()
    f = open("labelset_test", 'wb')
    pickle.dump(tes_gt, f)
    f.close()
